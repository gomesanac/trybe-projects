  ...
  function DPX(input) {
    ...
    function addEvents(dpx) {
      ...
      elements.content.addEventListener("click", function(e) {
        if (mode === 2) {
          dpx.setValue(e.target.dpxValue) &&
    ...
    function draw(dt) {
      ...
      // today button
      options.todayButton &&
        elements.today.classList[
          current >= dtMin && current <= dtMax ? "remove" : "add"
        ]("dpx-disabled");
      ...
      elements.title.innerHTML = mode
        ? (mode === 2 ? options.singleMonthLabels[setMonth] + " " : "") +
          setYear
        : zeroYear + " - " + (zeroYear + 9);
      elements.title.dpxValue = dt.getTime();
      elements.title.title =
        mode === 2 ? setYear : zeroYear + " - " + (zeroYear + 9);

      // prev and next arrows
      elements.prevTitle.classList[dt.getTime() <= dtMin ? "add" : "remove"](
        "dpx-disabled"
      );
      mode === 2
      ...
      elements.prevTitle.title = mode
        ? (mode === 2 ? options.singleMonthLabels[dt.getMonth()] + " " : "") +
          dt.getFullYear()
      ...
      elements.nextTitle.classList[dt.getTime() > dtMax ? "add" : "remove"](
        "dpx-disabled"
      );
      ...
      for (; i--; dt[setter](dt[getter]() + 1)) {
        ...
        (dt.getTime() < dtMin || dt.getTime() > dtMax) &&
          classes.push("dpx-disabled");
    ...
    return {
      ...
      getValue: function(timestamp) {
        !inited &&
          console.error("DatePickerX, getValue: Date picker doesn't init yet.");

        return timestamp
          ? value
          : value === null
          ? ""
          : getFormatedDate(new Date(value), options.format);
      },
